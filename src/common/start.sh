...
function send_ntfy_success {
    /app/common/ntfy.py -u $NTFY_URL -T $NTFY_SUCCESS_TITLE -m $NTFY_SUCCESS_MESSAGE -p $NTFY_SUCCESS_PRIORITY
}
# helper function to send ntfy push notification on failure
function send_ntfy_failure {
    /app/common/ntfy.py -u $NTFY_URL -T $NTFY_FAILURE_TITLE -m $NTFY_FAILURE_MESSAGE -p $NTFY_FAILURE_PRIORITY
}
