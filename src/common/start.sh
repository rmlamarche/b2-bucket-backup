...
function send_ntfy_success {
    ...
    /bbb/common/ntfy.py -u "${NTFY_URL}" -T "${NTFY_SUCCESS_TITLE}" -m "${time_elapsed}" -p "${NTFY_SUCCESS_PRIORITY}"
}
# helper function to send ntfy push notification on failure
function send_ntfy_failure {
    local time_elapsed="${1}"
    /bbb/common/ntfy.py -u "${NTFY_URL}" -T "${NTFY_FAILURE_TITLE}" -m "${time_elapsed}" -p "${NTFY_FAILURE_PRIORITY}"
}
...
RESULT=$?
DURATION=$SECONDS
TIME_ELAPSED="duration: $((DURATION / 60))m $((DURATION % 60))s"
